...
async function sendResetPassword(email, resetCode){
  const mailOptions = {
    from: '"KnightBites" <knightbitespoosd@gmail.com>',
    to: email,
  ...
  try {
    ...
    console.error('Failed to Reset-Password email:', err);
  }
}
...
app.post('/api/test-email', async (req, res) => {
  const { email, code } = req.body;  // Retrieve email and code from request body

  try {
    await sendVerificationEmail(email, code);
    res.status(200).json({ success: `Verification email sent to ${email}` });
  } catch (error) {
...
app.post('/api/addrecipe', async (req, res, next) => {
  ...
  const { userId, fullName, recipeName, recipeIngredients, imageEncoding } = req.body;  // _id is the userId in this case
  const newRecipe = { recipeName: recipeName, recipeIngredients: recipeIngredients, userId: userId, fullName: fullName, imageEncoding: imageEncoding };  // Link recipe to userId
  var error = '';
  var success = '';
...
app.post('/api/verify-email', async (req, res) => {
  ...
  try {
    ...
    const user = await db.collection('users').findOne({ email, verificationCode });

    if (user) {
      // Update user as verified
      const result = await db.collection('users').updateOne({ email },{ $set: { verified: true }, $unset: { verificationCode: '' } }
      );
...
app.post('/api/searchrecipes', async (req, res, next) => {
  ...
  try {
    ...
    // Search for recipes by recipeName, sorted in descending order
    const results = await db.collection('recipes')
      .find({
        recipeName: { $regex: _search, $options: 'i' } // Case-insensitive search
      })
      .sort({ _id: -1 }) // Sort by _id in reverse order
      .toArray();

    // Return the full recipe objects in reversed order
...
app.post('/api/displayRecipes', async (req, res, next) => {
  ...
  try {
    ...
    const results = await db.collection('recipes').find({ userId: userId }).sort({ _id: -1 }).toArray();
...
app.post('/api/favoritesAdd', async (req, res) => {
...
app.post('/api/favoritesDelete', async (req, res) => {
  const { userId, recipeId } = req.body;
...
app.post('/api/favoritesGet', async (req, res) => {
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await db.collection('users').updateOne(
      { email },
      { $set: { resetCode, resetCodeExpiration: expiration } }
    );
...
app.post('/api/reset-password', async (req, res) => {
  ...
  try {
    const db = client.db('KnightBites');
    const user = await db.collection('users').findOne({ email, resetCode });
    ...
    // Update user's password and clear the reset code and expiration
    await db.collection('users').updateOne(
      { email },
      { $set: { password: hashedPassword }, $unset: { resetCode: '', resetCodeExpiration: '' } }
    );
